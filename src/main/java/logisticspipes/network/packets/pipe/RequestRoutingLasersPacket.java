...
public class RequestRoutingLasersPacket extends CoordinatesPacket {
    ...
    private void handleRouteInDirection(
        ...
        while (!worklist.isEmpty()) {
            ...
            lasers.add(
                    new LaserData(pipe.xCoord, pipe.yCoord, pipe.zCoord, dir, connectionType).setStartPipe(firstPipe));
            firstPipe = false;
            HashMap<CoreRoutedPipe, ExitRoute> map = PathFinder.paintAndgetConnectedRoutingPipes(
                    pipe,
                    dir,
            ...
            for (Entry<CoreRoutedPipe, ArrayList<ExitRoute>> connectedPipe : sort.entrySet()) {
                HashMap<ForgeDirection, ArrayList<ExitRoute>> routers =
                        new HashMap<ForgeDirection, ArrayList<ExitRoute>>();
            ...
            do {
                ...
                while (iOptions.hasNext()) {
                    ...
                    if (d.getPosX() == next.getX() && d.getPosY() == next.getY() && d.getPosZ() == next.getZ()) {
                        if (data.getDir().equals(d.getDir())
                                && data.getConnectionType().equals(d.getConnectionType())) {
