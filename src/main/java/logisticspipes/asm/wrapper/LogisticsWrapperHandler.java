        ...
        synchronized (lookupMap) {
            ...
            if (clazz == null) {
                ...
                for (Method method : interfaze.getMethods()) {
                    LogisticsWrapperHandler.addProxyMethod(
                            cw,
                            method,
                            fieldName,
                            className,
                            lineAddition,
                            !wrapperInterfacesList.contains(method.getReturnType()));
                    lineAddition += 10;
                }
                ...
                try {
                    ...
                    try {
                        if (e.getMessage().contains("attempted")
            ...
            if (clazz == null) {
                ...
                }
                {
                    MethodVisitor mv = cw.visitMethod(
                            Opcodes.ACC_PUBLIC,
                            "<init>",
                            "(Llogisticspipes/asm/wrapper/AbstractWrapper;L" + fieldName + ";L" + fieldName + ";)V",
                            null,
                            null);
                    mv.visitCode();
                    Label l0 = new Label();
                ...
                for (Method method : interfaze.getMethods()) {
                    LogisticsWrapperHandler.addProxyMethod(
                            ...
                            className,
                            lineAddition,
                            !wrapper.getWrapperInterfaces().contains(method.getReturnType()));
                    lineAddition += 10;
                }
                cw.visitEnd();
