...
public class PipeItemsInvSysConnector extends CoreRoutedPipe
            ...
            for (ItemIdentifier ident : items) {
                ...
                for (Iterator<ItemRoutingInformation> iterator = needs.iterator(); iterator.hasNext();) {
                    ...
                    if (need.getItem().getStackSize() <= itemAmount) {
                        ...
                        iterator.remove(); // finished with this need, we sent part of a stack, lets see if anyone where
                        // needs the current item type.
                        contentchanged = true;
                        if (needs.isEmpty()) {
