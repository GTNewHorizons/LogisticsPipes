...
public class LogisticsHUDRenderer {
    ...
    public void renderWorldRelative(long renderTicks, float partialTick) {
        ...
        if (list.size() == 0
                || Math.hypot(lastXPos - player.posX, Math.hypot(lastYPos - player.posY, lastZPos - player.posZ)) > 0.5
                || (renderTicks % 10 == 0
                        && (lastXPos != player.posX || lastYPos != player.posY || lastZPos != player.posZ))
                || renderTicks % 600 == 0) {
            ...
            if (renderer.getRenderer().display(config)) {
                ...
                if (!cursorHandled) {
                    ...
                    if (Math.hypot(x, Math.hypot(y, z)) < 0.75
                            || (renderer instanceof IHeadUpDisplayBlockRendererProvider
                                    && (((IHeadUpDisplayBlockRendererProvider) renderer).isHUDInvalid()
                                            || !((IHeadUpDisplayBlockRendererProvider) renderer).isHUDExistent()))) {
                        refreshList(player.posX, player.posY, player.posZ);
                        GL11.glPopMatrix();
                        break;
                    }
            ...
            if (progress != 0) {
                ...
                if (!textData.isEmpty()) {
                    ...
                    GuiGraphics.drawGuiBackGround(
                            mc,
                            (int) ((-0.5 * (width - 32)) * dProgress) - 16,
                            (int) ((-0.5 * (heigth - 32)) * dProgress) - 16,
                            (int) ((0.5 * (width - 32)) * dProgress) + 16,
                            (int) ((0.5 * (heigth - 32)) * dProgress) + 16,
                            0,
                            false);
