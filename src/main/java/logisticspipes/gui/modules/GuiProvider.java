...
public class GuiProvider extends ModuleBaseGui {
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiStringHandlerButton(
                0, width / 2 + 40, height / 2 - 59, 45, 20, new GuiStringHandlerButton.StringHandler() {
    ...
    protected void actionPerformed(GuiButton guibutton) {
        if (guibutton.id == 0) {
            ...
            MainProxy.sendPacketToServer(
                    PacketHandler.getPacket(ProviderModuleIncludePacket.class).setModulePos(_provider));
        } else if (guibutton.id == 1) {
            _provider.nextExtractionMode();
            MainProxy.sendPacketToServer(
                    PacketHandler.getPacket(ProviderModuleNextModePacket.class).setModulePos(_provider));
