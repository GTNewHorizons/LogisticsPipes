...
public class GuiCraftingPipe extends ModuleBaseGui {
    ...
    public GuiCraftingPipe(
        ...
        for (int i = 0; i < liquidCrafter; i++) {
            ...
            fluidSlotIDs[i] = extentionControllerLeft.registerControlledSlot(
                    dummy.addFluidSlot(i, module.getFluidInventory(), liquidLeft + 11, 24));
        }
    ...
    public void initGui() {
        ...
        if (!isAdvancedSat) {
            ...
            addButton(
                    normalButtonArray[0] =
                            new SmallGuiButton(0, (width - xSize) / 2 + 155, (height - ySize) / 2 + 50, 10, 10, ">"));
            addButton(
                    normalButtonArray[1] =
                            new SmallGuiButton(1, (width - xSize) / 2 + 120, (height - ySize) / 2 + 50, 10, 10, "<"));
            addButton(
            ...
            addButton(
                    normalButtonArray[4] =
                            new SmallGuiButton(20, (width - xSize) / 2 + 155, (height - ySize) / 2 + 85, 10, 10, ">"));
            addButton(
                    normalButtonArray[5] =
                            new SmallGuiButton(21, (width - xSize) / 2 + 120, (height - ySize) / 2 + 85, 10, 10, "<"));
            if (liquidCrafter != 0) {
                extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                        normalButtonArray[6] = new SmallGuiButton(
                                22, guiLeft - (liquidCrafter * 40) / 2 + 5, guiTop + 158, 10, 10, ">"))));
                extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                        normalButtonArray[7] = new SmallGuiButton(
                                23, guiLeft - (liquidCrafter * 40) / 2 - 15, guiTop + 158, 10, 10, "<"))));
            }
        } else {
            for (int i = 0; i < 9; i++) {
                addButton(
                        advancedSatButtonArray[i][0] = new SmallGuiButton(
                                30 + i, (width - xSize) / 2 + 10 + 18 * i, (height - ySize) / 2 + 40, 15, 10, "/\\"));
                addButton(
                        advancedSatButtonArray[i][1] = new SmallGuiButton(
                                40 + i, (width - xSize) / 2 + 10 + 18 * i, (height - ySize) / 2 + 70, 15, 10, "\\/"));
            }
            addButton(
                    normalButtonArray[2] = new SmallGuiButton(
                            3,
            ...
            addButton(
                    normalButtonArray[4] =
                            new SmallGuiButton(20, (width - xSize) / 2 + 155, (height - ySize) / 2 + 105, 10, 10, ">"));
            addButton(
                    normalButtonArray[5] =
                            new SmallGuiButton(21, (width - xSize) / 2 + 120, (height - ySize) / 2 + 105, 10, 10, "<"));
        }
        for (int i = 0; i < liquidCrafter; i++) {
            if (isAdvancedSat) {
            ...
            extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    liquidGuiParts[i][2] =
                            new SmallGuiButton(100 + 10 * i + 2, liquidLeft + 22, guiTop + 105, 10, 10, "+"))));
            extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    liquidGuiParts[i][3] =
                            new SmallGuiButton(100 + 10 * i + 3, liquidLeft + 22, guiTop + 125, 10, 10, "+"))));
            extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    liquidGuiParts[i][4] =
                            new SmallGuiButton(100 + 10 * i + 4, liquidLeft + 8, guiTop + 65, 10, 10, "-"))));
            extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    liquidGuiParts[i][5] =
                            new SmallGuiButton(100 + 10 * i + 5, liquidLeft + 8, guiTop + 85, 10, 10, "-"))));
            extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    liquidGuiParts[i][6] =
                            new SmallGuiButton(100 + 10 * i + 6, liquidLeft + 8, guiTop + 105, 10, 10, "-"))));
            extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    liquidGuiParts[i][7] =
                            new SmallGuiButton(100 + 10 * i + 7, liquidLeft + 8, guiTop + 125, 10, 10, "-"))));
            if (isAdvancedSat) {
                extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                        liquidGuiParts[i][8] =
                                new SmallGuiButton(100 + 10 * i + 8, liquidLeft + 5, guiTop + 158, 10, 10, "<"))));
                extention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                        liquidGuiParts[i][9] =
                                new SmallGuiButton(100 + 10 * i + 9, liquidLeft + 25, guiTop + 158, 10, 10, ">"))));
        ...
        if (cleanupSize > 0) {
            ...
            cleanupExtention.registerButton(extentionControllerLeft.registerControlledButton(addButton(
                    cleanupModeButton = new SmallGuiButton(
                            ...
                            StringUtils.translate(
                                    GuiCraftingPipe.PREFIX + (_pipe.cleanupModeIsExclude ? "Exclude" : "Include"))))));
            cleanupExtention.registerButton(
                    extentionControllerLeft.registerControlledButton(addButton(new SmallGuiButton(
                            25,
                            guiLeft - 56,
        ...
        switch (guibutton.id) {
            ...
            case 24:
                MainProxy.sendPacketToServer(
                        PacketHandler.getPacket(CPipeCleanupToggle.class).setModulePos(_pipe));
                return;
            case 25:
                MainProxy.sendPacketToServer(
                        PacketHandler.getPacket(CPipeCleanupImport.class).setModulePos(_pipe));
                return;
            default:
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        mc.fontRenderer.drawString(
                StringUtils.translate(GuiCraftingPipe.PREFIX + "Inventory"), 10, ySize - 93, 0x404040);
        ...
        if (!isAdvancedSat) {
            ...
            mc.fontRenderer.drawString(
                    StringUtils.translate(GuiCraftingPipe.PREFIX + "Priority") + ":", 123, 75, 0x404040);
            mc.fontRenderer.drawString(
                    "" + _pipe.priority, 143 - (mc.fontRenderer.getStringWidth("" + _pipe.priority) / 2), 87, 0x404040);
        } else {
            for (int i = 0; i < 9; i++) {
                if (_pipe.advancedSatelliteIdArray[i] == 0) {
                    mc.fontRenderer.drawString(
                            StringUtils.translate(GuiCraftingPipe.PREFIX + "Off"), 10 + (i * 18), 57, 0x404040);
                } else {
                    mc.fontRenderer.drawString(
            ...
            mc.fontRenderer.drawString(
                    StringUtils.translate(GuiCraftingPipe.PREFIX + "Priority") + ":", 123, 95, 0x404040);
            mc.fontRenderer.drawString(
                    "" + _pipe.priority,
    ...
    public void onCleanupModeChange() {
        cleanupModeButton.displayString =
                StringUtils.translate(GuiCraftingPipe.PREFIX + (_pipe.cleanupModeIsExclude ? "Exclude" : "Include"));
    }
        ...
        public void renderForground(int left, int top) {
            ...
            if (!isAdvancedSat && liquidCrafter > 1 && !isFullyExtended()) {
                ...
                mc.fontRenderer.drawStringWithShadow(
                        s, left + 22 - fontRendererObj.getStringWidth(s), top + 14, 16777215);
            }
            if (isFullyExtended()) {
        ...
        private void renderFluidText(int left, int top, int i) {
            ...
            if (isAdvancedSat) {
                if (_pipe.liquidSatelliteIdArray[i] == 0) {
                    ...
                    mc.fontRenderer.drawString(
                            StringUtils.translate(GuiCraftingPipe.PREFIX + "Off"), left + 15, top + 146, 0x404040);
                    for (int j = 0; j < 8; j++) {
                        liquidGuiParts[i][j].enabled = false;
                    }
                } else {
    ...
    private final class ByproductExtention extends GuiExtention {
        ...
        public void renderForground(int left, int top) {
            if (!isFullyExtended()) {
                ...
                RenderHelper.enableGUIStandardItemLighting();
                ItemStack stack =
                        new ItemStack(LogisticsPipes.UpgradeItem, 1, ItemUpgrade.CRAFTING_BYPRODUCT_EXTRACTOR);
                GuiScreen.itemRender.renderItemAndEffectIntoGUI(
                        fontRendererObj, getMC().renderEngine, stack, left + 5, top + 5);
                GuiScreen.itemRender.renderItemOverlayIntoGUI(
                        fontRendererObj, getMC().renderEngine, stack, left + 5, top + 5, "");
                GL11.glDisable(GL11.GL_LIGHTING);
                GL11.glDisable(GL11.GL_DEPTH_TEST);
                ...
                fontRendererObj.drawString(
                        StringUtils.translate(GuiCraftingPipe.PREFIX + "Extra"), left + 9, top + 8, 0x404040);
            }
        }
    }
