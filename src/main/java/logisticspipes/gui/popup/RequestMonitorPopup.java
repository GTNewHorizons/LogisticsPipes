        ...
        for (int x = left; x < right + width; x += width) {
            ...
            for (int y = top; y < bottom + height; y += height) {
                ...
                bufferedimage.setRGB(
                        imgPosX,
                        imgPosY,
                        Math.min(useWidth, bufferedimage.getWidth() - imgPosX),
                        Math.min(useHeight, bufferedimage.getHeight() - imgPosY),
                        intArray,
                        0,
                        useWidth);
                imgPosY += useHeight;
    ...
    private void drawMap(int par1, int par2) {
        ...
        if (!list.isEmpty()) {
            SimpleGraphics.drawVerticalLine(
                    innerLeftSide - mapX + 110,
                    innerTopSide - mapY - 197,
                    innerTopSide - mapY - 180,
                    Color.GREEN,
                    zoom.line);
        }
        ...
        if (!list.isEmpty()) {
            ...
            mc.fontRenderer.drawStringWithShadow(
                    s,
                    innerLeftSide - mapX + 111 - mc.fontRenderer.getStringWidth(s) / 2,
                    innerTopSide - mapY - 210,
                    16777215);
        } else {
