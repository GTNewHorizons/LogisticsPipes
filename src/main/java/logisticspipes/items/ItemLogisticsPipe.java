...
public class ItemLogisticsPipe extends LogisticsItem {
    ...
    /**
     * Adds all keys from the translation file in the format: item.className.tip([0-9]*) Tips start from 1 and
     * increment. Sparse rows should be left empty (ie empty line must still have a key present) Shift shows full
     * tooltip, without it you just get the first line.
     */
    @Override
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z,
        ...
        if (world.canPlaceEntityOnSide(block, i, j, k, false, side, entityplayer, itemstack)) {
            ...
            if (pipe == null) {
                LogisticsPipes.log.log(Level.WARN, "Pipe failed to create during placement at {},{},{}", i, j, k);
                return true;
            }
