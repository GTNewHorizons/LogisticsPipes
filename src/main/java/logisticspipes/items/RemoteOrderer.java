...
public class RemoteOrderer extends Item {
                ...
                if (pipe.useEnergy(energyUse)) {
                    MainProxy.sendPacketToPlayer(
                            PacketHandler.getPacket(RequestPipeDimension.class)
                                    .setInteger(MainProxy.getDimensionForWorld(pipe.getWorld())),
                            par3EntityPlayer);
    ...
    public static PipeItemsRemoteOrdererLogistics getPipe(ItemStack stack) {
        ...
        TileEntity tile = world.getTileEntity(
                stack.stackTagCompound.getInteger("connectedPipe-x"),
                stack.stackTagCompound.getInteger("connectedPipe-y"),
                stack.stackTagCompound.getInteger("connectedPipe-z"));
