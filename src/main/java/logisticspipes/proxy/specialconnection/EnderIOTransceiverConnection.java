...
public class EnderIOTransceiverConnection implements ISpecialTileConnection {
    ...
    public Collection<TileEntity> getConnections(TileEntity tile) {
        ...
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (candidate instanceof LogisticsTileGenericPipe
                    && MainProxy.checkPipesConnections(tile, candidate, direction)) {
                if (onlyOnePipe) {
                    onlyOnePipe = false;
                    break;
                } else {
        ...
        for (TileEntity connected : connections) {
            ...
            for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
                ...
                TileEntity candidate = p.getTileEntity(tile.getWorldObj());
                if (candidate instanceof LogisticsTileGenericPipe
                        && MainProxy.checkPipesConnections(connected, candidate, direction)) {
                    if (pipe != null) {
                        pipe = null;
                        break;
                    } else {
    ...
    public void transmit(TileEntity tile, IRoutedItem data) {
        ...
        if (pipe instanceof LogisticsTileGenericPipe) {
            ((CoreRoutedPipe) ((LogisticsTileGenericPipe) pipe).pipe)
                    .queueUnroutedItemInformation(data.getItemIdentifierStack().clone(), data.getInfo());
        } else {
            new RuntimeException("Only LP pipes can be next to transceiver to queue item information")
                    .printStackTrace();
        }
    }
}
