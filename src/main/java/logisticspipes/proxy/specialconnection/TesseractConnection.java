...
public class TesseractConnection implements ISpecialTileConnection {
    ...
    public List<TileEntity> getConnections(TileEntity tile) {
        ...
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (canidate instanceof LogisticsTileGenericPipe
                    && MainProxy.checkPipesConnections(tile, canidate, direction)) {
                if (onlyOnePipe) {
                    onlyOnePipe = false;
                    break;
                } else {
        ...
        for (TileEntity connected : connections) {
            ...
            for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (canidate instanceof LogisticsTileGenericPipe
                        && MainProxy.checkPipesConnections(connected, canidate, direction)) {
                    if (pipe != null) {
                        pipe = null;
                        break;
                    } else {
