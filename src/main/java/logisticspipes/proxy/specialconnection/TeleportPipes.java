...
public class TeleportPipes implements ISpecialPipedConnection {
    ...
    public boolean init() {
        try {
            try {
                TeleportPipes.PipeItemTeleport =
                        (Class<? extends Pipe>) Class.forName("buildcraft.additionalpipes.pipes.PipeItemTeleport");
            } catch (Exception e) {
                TeleportPipes.PipeItemTeleport = (Class<? extends Pipe>)
                        Class.forName("net.minecraft.src.buildcraft.additionalpipes.pipes.PipeItemTeleport");
            }
            TeleportPipes.teleportPipeMethod =
                    TeleportPipes.PipeItemTeleport.getMethod("getConnectedPipes", boolean.class);
            LogisticsPipes.log.debug("Additional pipes detected, adding compatibility");
            return true;
        } catch (Exception e1) {
            try {
                TeleportPipes.PipeItemTeleport =
                        (Class<? extends Pipe>) Class.forName("buildcraft.additionalpipes.pipes.PipeItemsTeleport");
                ...
                TeleportPipes.teleportPipeMethod = tpmanager.getMethod(
                        "getConnectedPipes",
                        Class.forName("buildcraft.additionalpipes.pipes.PipeTeleport"),
                        boolean.class);
                LogisticsPipes.log.debug("Additional pipes detected, adding compatibility");
                return true;
            } catch (Exception e2) {
    ...
    private LinkedList<? extends Pipe> getConnectedTeleportPipes(Pipe pipe) throws Exception {
        if (TeleportPipes.teleportManager != null) {
            return (LinkedList<? extends Pipe>)
                    TeleportPipes.teleportPipeMethod.invoke(TeleportPipes.teleportManager, pipe, false);
        }
            ...
            try {
                ...
                for (Pipe pipe : pipes) {
                    list.add(new ConnectionInformation(
                            SimpleServiceLocator.pipeInformationManager.getInformationProviderFor(pipe.container),
                            connection,
                            side,
