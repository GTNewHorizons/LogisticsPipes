...
public class ItemInsertionAIRobot extends AIRobot {
    ...
    public ItemInsertionAIRobot(
            EntityRobotBase iRobot,
            LogisticsTileGenericPipe pipe,
            LogisticsRoutingBoardRobot board,
            ForgeDirection insertion) {
        super(iRobot);
        this.pipe = pipe;
        this.board = board;
            ...
            if (iter.hasNext()) {
                ...
                robotInv.getMultipleItems(
                        ...
                        item.getItemIdentifierStack()
                                .getStackSize()); // We clear the Inv at the end so we don't care if this fails
                iter.remove();
            } else {
                terminate();
            }
