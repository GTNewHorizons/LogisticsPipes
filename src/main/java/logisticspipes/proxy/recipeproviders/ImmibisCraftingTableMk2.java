    ...
    public boolean importRecipe(TileEntity tile, ItemIdentifierInventory inventory) {
        try {
            if (tileAutoCraftingMk2.isInstance(tile)) {
                ...
                ItemStack[][] recipe = (ItemStack[][])
                        tileAutoCraftingMk2.getField("recipeInputs").get(tile);
                // Not really a AutoCraftingInventory, but same content
                InventoryCrafting tempCraftingInv = new InventoryCrafting(
                        new Container() {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    if (slotStack != null && slotStack.getItem() != null) {
                        ...
                        for (int j = i + 1; j < 9; j++) {
                            ...
                            if (tempStack != null
                                    && ItemIdentifier.get(slotStack).equals(ItemIdentifier.get(tempStack))) {
