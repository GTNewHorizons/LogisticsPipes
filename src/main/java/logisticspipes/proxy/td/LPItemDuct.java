    ...
    public void transferItem(TravelingItem item) {
        ...
        if (info != null) {
            ...
            if (item.stack.stackSize > 0) {
                pipe.pipe.transport._itemBuffer.add(new Triplet<
                        ItemIdentifierStack, Pair<Integer, Integer>, LPTravelingItemServer>(
                        ItemIdentifierStack.getFromStack(item.stack), new Pair<Integer, Integer>(20 * 2, 0), null));
            }
        }
    }
