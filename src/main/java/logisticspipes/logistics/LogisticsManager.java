...
public class LogisticsManager implements ILogisticsManager {
    ...
    private Triplet<Integer, SinkReply, List<IFilter>> getBestReply(ItemIdentifier stack, IRouter sourceRouter,
        ...
        outer: for (ExitRoute candidateRouter : validDestinations) {
            ...
            if (reply.fixedPriority == result.getValue2().fixedPriority
    ...
    public IRoutedItem assignDestinationFor(IRoutedItem item, int sourceRouterID, boolean excludeSource) {
        ...
        if (item.getItemIdentifierStack() != null
                && item.getItemIdentifierStack().makeNormalStack().getItem() instanceof LogisticsFluidContainer) {
            Pair<Integer, Integer> bestReply = SimpleServiceLocator.logisticsFluidManager.getBestReply(
                    SimpleServiceLocator.logisticsFluidManager.getFluidFromContainer(item.getItemIdentifierStack()),
                    sourceRouter,
                    item.getJamList());
            ...
            Triplet<Integer, SinkReply, List<IFilter>> bestReply = getBestReply(
