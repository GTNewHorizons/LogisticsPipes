...
public class DebugController implements IRoutingDebugAdapter {
    ...
    public void start(
        ...
        MainProxy.sendPacketToPlayer(
                PacketHandler.getPacket(RoutingUpdateDebugCanidateList.class).setMsg(e), (EntityPlayer) sender);
        wait("Start?", true);
    }
    ...
    public void handledPipe(boolean flag) {
        for (int i = 0; i < closedSet.size(); i++) {
            ...
            if (set != null) {
                ...
                if (router != null) {
                    MainProxy.sendPacketToPlayer(
                            PacketHandler.getPacket(RoutingUpdateDebugClosedSet.class)
                                    .setPos(router.getLPPosition())
                                    .setSet(set),
                            (EntityPlayer) sender);
                }
            }
        ...
        for (int i = 0; i < filterList.size(); i++) {
            ...
            if (filters != null) {
                ...
                if (router != null) {
                    MainProxy.sendPacketToPlayer(
                            PacketHandler.getPacket(RoutingUpdateDebugFilters.class)
                                    .setPos(router.getLPPosition())
                                    .setFilters(filters),
                            (EntityPlayer) sender);
                }
