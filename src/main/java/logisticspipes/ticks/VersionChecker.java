        ...
        if (versionCheckFuture.isDone()) {
            try {
                ...
                LogisticsPipes.log.warn(
                        "The version check task had an exception while getting the newest version information", e);
                    ...
                    for (String commit : changelog.get(build)) {
                        if (commit.length() > COMMIT_MAX_LINE_LENGTH) {
                            ...
                            while (!commit.isEmpty()) {
                                ...
                                int splitAt = commit.substring(0, Math.min(maxLength, commit.length()))
                                        .lastIndexOf(' ');
                                if (commit.length() < COMMIT_MAX_LINE_LENGTH) {
                                ...
                                } else if (commit.length() > COMMIT_MAX_LINE_LENGTH
                                        && splitAt < COMMIT_MAX_LINE_LENGTH - 20) {
                                    splitAt = Math.min(maxLength, commit.length());
                                }
